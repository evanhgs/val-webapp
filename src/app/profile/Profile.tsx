  ...
  */
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        const endpoint = isOwnProfile ? Config.user.currentUserProfile() : Config.user.profile(targetUsername);
    ...
    const fetchProfileData = async () => {
      try {
        ...
        const [followedResponse, followerResponse, postResponse] = await Promise.all([ // ordre very important ^^
          AxiosInstance.get(Config.follow.getFollowed(userData.username)),
          AxiosInstance.get(Config.follow.getFollowers(userData.username)),
          AxiosInstance.get(Config.post.feed(userData.username))
        ]);
        ...
        <div className="max-w-4xl mx-auto p-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:space-x-8">
            ...
            <div className="flex justify-center sm:justify-start mb-6 sm:mb-0">
              <img
                src={userData.profile_picture ? Config.user.picture(userData.profile_picture) : Config.user.defaultPicture()}
                alt="Profile"
