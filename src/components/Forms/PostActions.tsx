...
import {AxiosInstance, Config, FRONTEND_URL} from "../../config/config.ts";
...
export const PostActions = ({ postId }: { postId: number}) => {
        ...
        const fetchLikes = async () => {
            const response = await AxiosInstance.get(Config.like.getPostLikes(postId))
            setLikeContent({
        ...
        const fetchComments = async () => {
            const response = await AxiosInstance.get(Config.comment.getComments(postId))
            setCommentContent({
        ...
        const likePost = async () => {
            ...
            try {
                await AxiosInstance.post(Config.like.likePost(postId));
        ...
        const unlikePost = async () => {
            ...
            try {
                await AxiosInstance.delete(Config.like.unlikePost(postId));
                                            ...
                                            <li>
                                                <div className="max-w-full rounded-full ml-3 cursor-pointer" onClick={() => { navigate(`/profile/${u.username}`) }}>
                                                    <img
                                                        ...
                                                        src={u.profile_picture ? Config.user.picture(u.profile_picture) : Config.user.defaultPicture()}
                                                    />
                                                    <a>{u.username}</a>
                                                </div>
                                                ...
                                                <div key={c.id} className="chat chat-start mb-4">
                                                    <div className="chat-image avatar">
                                                        <div className="w-10 rounded-full">
                                                            <img
                                                                alt="Tailwind CSS chat bubble component"
                                                                src={c.user.profile_picture ? Config.user.picture(c.user.profile_picture) : Config.user.defaultPicture()}
                                                            />
                                                        </div>
                                                    </div>
                                ...
                                <div className="flex items-center border-t border-gray-800 p-3">
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            AxiosInstance.post(Config.comment.addComment(postId), {
                                                content: value,
                                            })
                                                .then(() => {
